                    ...
                    <el-row :class="['bdbottom', i1===0?'.bdtop':'', 'vcenter']" v-for="(item1, i1) in scope.row.children" :key="item1.id">
                        ...
                        <el-col :span="19">
                            ...
                            <el-row :class="[i2===0?'':'bdtop', 'vcenter']" v-for="(item2, i2) in item1.children" :key="item2.id">
                                ...
                                <!-- 三级权限 -->
                                <el-col :span="18">
                                    <el-tag :class="[i3===0?'':'']" type = "warning" v-for="(item3,i3) in item2.children" :key="item3.id" closable @close="removeRightById(scope.row, item3.id)">{{item3.authName}}</el-tag> <!--eslint-disable-line no-unused-vars-->
                                </el-col>
                            </el-row>
                        </el-col>
                    </el-row>
...
export default {
    ...
    //展示分配权限对话框
    async showSetRightDialog(role) {//形参传入role
        const {data: res}=await this.$http.get('rights/tree');//使用树形结构
        if(res.meta.status != 200){
    ...
    allotRights() {
        const keys = [
            ...this.$refs.getCheckedKeys(),//通过展开运算符加入数组
            ...this.$refs.getHalfCheckedKeys(),
        ];
        console.log(keys);
    }
    }
